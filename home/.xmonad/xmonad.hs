...
myConfig = def
  ...
  -- launch dmenu
  [ ((myModMask,                xK_r     ), spawn ("dmenu_run " ++ myDmenuArgs))
  -- launch j4-dmenu-desktop
  , ((myModMask .|. shiftMask,  xK_r     ), spawn ("j4-dmenu-desktop --dmenu=\"dmenu -i " ++ myDmenuArgs ++ "\" --term=\"" ++ myTerminal ++ "\""))

  -- launch browser
  , ((myModMask .|. shiftMask,  xK_x     ), spawn (myBrowser ++ " >/dev/null 2>&1"))
  ...
  -- toggle redshift
  , ((myModMask .|. shiftMask,  xK_p                    ), spawn myToggleRedshiftCmd)
